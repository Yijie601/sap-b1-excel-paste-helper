...
public static class AppLogger
{
    ...
    public static void Success(string supplier, string code, string documentNumber, int rows, TimeSpan duration) =>
        Write("SUCCESS", supplier, code, documentNumber, rows, duration, null, null);

    public static void Failure(string? supplier, string? code, string? documentNumber, int rows, TimeSpan duration, string error) =>
        Write("ERROR", supplier, code, documentNumber, rows, duration, error, null);

    public static void Error(string status, string message, Exception? exception = null) =>
        Write(status, null, null, null, 0, TimeSpan.Zero, message, exception);
